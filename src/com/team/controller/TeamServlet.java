	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/team/select_page.jsp");
				...
				TeamVO teamVO = teamSvc.getOneEmp(trip_id, users_id);
				if (teamVO == null) {
					errorMsgs.add("查無資料");
				}
				// Send the use back to the form, if there were errors
				...
				req.setAttribute("teamVO", teamVO); // 資料庫取出的empVO物件,存入req
				String url = "/team/listOneEmp.jsp";
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("teamVO", teamVO);         // 資料庫取出的empVO物件,存入req
				String url = "/team/update_emp_input.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/team/listAllEmp.jsp");
				failureView.forward(req, res);
			}
		}
...
//		}
        ...
        if ("insert".equals(action)) { // 來自addEmp.jsp的請求  
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
												.getRequestDispatcher("/team/addEmp.jsp");
										failureView.forward(req, res);
										return;
				}
				...
				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
req.setAttribute("teamVO", teamVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/team/addEmp.jsp");
					failureView.forward(req, res);
					return;
				...
				teamVO = teamSvc1.addEmp(trip_id, users_id);
				
				/***************************3.新增完成,準備轉交(Send the Success view)***********/
				String url = "/team/listAllEmp.jsp";
		...
		if ("delete".equals(action)) { // 來自listAllEmp.jsp
			...
			try {
				...
				/***************************2.開始刪除資料***************************************/
				TeamService teamSvc = new TeamService();
				teamSvc.deleteEmp(trip_id ,  users_id);
