...
public class LoginDAO implements Login_interface{
	...
	private static final String createView= 
			...
			private static final String userslogin= 
					"select users_id from users where users_mail like (?) and users_pwd like (?)";
			...
			public SaleVO usersLogin(String account,String pass) {
				...
				try {
					...
					pstmt = con.prepareStatement(userslogin);
				...
				//	pstmt.setInt(1, trip_id);
					...
					if (rs.next()) {
						uVO=new SaleVO();
						uVO.setSale_id(rs.getInt("users_id"));
