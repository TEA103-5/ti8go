...
<%-- 								class="com.orderDetail.model.OrderDetailService" /> --%>
							...
							<!-- 							<tr> -->
							<!-- 								<td>訂單+明細:<font color=red><b>*</b></font></td> -->
							<!-- 								<td><select size="1" name="users_id"> -->
<%-- 							<c:forEach var="dpt1VO" items="${detailSvc.all}"> --%>
								<%-- 											<option value="${dpt1VO.users_id}" --%>
								<%-- 												${(orderVO.users_id==deptVO.users_id)? 'selected':'' }>${dptVO.users_name} --%>
<%-- 							</c:forEach> --%>
							<!-- 								</select></td> -->
							<!-- 							</tr> -->
					...
					<!-- 						<input type="button" id="Submit" name="Submit" value="購買" /> -->
					<!-- 					</form> -->
					<ul>
						<li><jsp:useBean id="detail1Svc" scope="page"
								class="com.orderDetail.model.OrderDetailService" />
							<FORM METHOD="post"
								ACTION="<%=request.getContextPath()%>/Order/Order.do"
								name="form1">
								<b>測試新增訂單:</b>
								<c:forEach var="dpt1VO" items="${detail1Svc.all}">
									<input type="hidden"  id="order_address2" name="order_address2" size="45"
										value="<%=(orderVO == null) ? "" : orderVO.getOrder_address()%>">
									<input type="button" value="購買" onclick="fun1(${dpt1VO.order_detail_count})">
									<p>Order <span id="displayCount">0</span> times.</p>
									
								</c:forEach>
								<br> <input type="hidden" name="action" value="insertWithDetail">
					        	<input type="submit" value="送出新增">
							</FORM></li>
					</ul>
				</div>
