...
<title>活動報名人資料新增 - addApplicant.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>活動報名人資料新增 - addApplicant.jsp</h3></td>
		 <td>
		 <h4><a href="<%=request.getContextPath()%>/applicant/select_page.jsp"><img src="images/tomcat.png" width="100" height="100" border="0">回首頁</a></h4>
	</td>
	</tr>
</table>
	...
	<tr>
		...
		<td><select size="1" name="users_id">
			<c:forEach var="usersVO" items="${usersSvc.all}">
				<option value="${usersVO.users_id}" ${(applicantVO.users_id==usersVO.users_id)? 'selected':'' } >${usersVO.users_id}-${usersVO.users_name}
			</c:forEach>
		</select></td>
	</tr>
