...
<title>活動資料新增 - addActivities.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>活動資料新增 - addGroup_activities.jsp</h3></td><td>
		 <h4><a href="<%=request.getContextPath()%>/group_activities/select_page.jsp"><img src="images/tomcat.png" width="100" height="100" border="0">回首頁</a></h4>
	</td></tr>
</table>
...
<table>
	<tr>
		<td>活動名稱:</td>
		<td><input type="TEXT" name="activities_name" size="45" 
			 value="<%= (group_activitiesVO==null)? "活動" : group_activitiesVO.getActivities_name()%>" /></td>
	</tr>

	<tr>
		<td>活動描述:</td>
		<td><input type="TEXT" name="activities_description" size="45"
			 value="<%= (group_activitiesVO==null)? "描述.." : group_activitiesVO.getActivities_description()%>" /></td>
	</tr>
	<tr>
		<td>報名截止日:</td>
	...
	<tr>
		<td>報名人數上限:</td>
		<td><input type="TEXT" name="activities_max_users" size="45"
			 value="<%= (group_activitiesVO==null)? "10" : group_activitiesVO.getActivities_max_users()%>" /></td>
	</tr>
	<tr>
		<td>活動狀態:</td>
		<td><input type="TEXT" name="activities_state" size="45"
			 value="<%= (group_activitiesVO==null)? "1" : group_activitiesVO.getActivities_state()%>" /></td>
	</tr>
	...
	<tr>
		...
		<td><select size="1" name="users_id">
			<c:forEach var="usersVO" items="${usersSvc.all}">
				<option value="${usersVO.users_id}" ${(group_activitiesVO.users_id==usersVO.users_id)? 'selected':'' } >${usersVO.users_id}-${usersVO.users_name}
			</c:forEach>
		</select></td>
	</tr>
...
<% 
  ...
  try {
	  end = group_activitiesVO.getActivities_end();
   } catch (Exception e) {
